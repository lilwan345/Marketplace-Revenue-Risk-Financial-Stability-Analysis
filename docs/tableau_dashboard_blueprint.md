...
| Patterns-to-monitor list | `risk_dimension`, `pattern_observation` | Text table or compact pattern cards sorted by `sort_order` |
...
| Action plan | `priority`, `action_area`, `pattern_observation`, `tracking_metric` | Text table sorted by `priority` |
